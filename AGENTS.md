...
> **开工前必读顺序**：**`金水谣_标准唯一真源.md`（先看这个，它告诉你下面每一篇谁说了算、哪些只是引用、数值阈值以代码常量为准）** → 交接中心 → 纲 → 契 → 录 → 总索引
> ⚠️ 同一份规范存在「仓库真源 / `模型\`根镜像 / `obsidian-vault\`导出 / `90_归档文档\`归档」四份副本，**只能编辑仓库真源**；改完必须 `Copy-Item` 覆盖根镜像并把 mtime 刷到 ≥ 真源（详见标准唯一真源 §一）。

---

## 🔴 铁律 -1：防再犯铁律清单（最高优先，开工必读）

**每次开工、每次改码前，先读 `tools/ai_guard_rules.md`**（高频错误清单，2026-08-04 固化）。
它是"反复解决同类问题"的解药：A.密钥/Git安全、B.代码修改规范、C.GUI交互(tkinter)、D.后台任务、E.同步文件、F.项目规范。
