...
| **🟢 智能探路引擎（链路地图）** | 新建 tools/route_probe.py 导航式探路: 6条链路31节点(串行A预测/B复盘/C大脑/D知识库, 并行E子系统/F审查保障), 逐节点真实探测(文件/数据/真实generate), 失败给修复tip+串行前节点失败则后续标"不可达"; --json 导出地图; 接 GET /api/chain-map(独立进程防阻塞) + 页面 /chain-map + 控制中心侧边栏"链路地图"入口; 验证: CLI 与 API 均 6链路5通1断(E6足彩数据未接入, 如实标注) exit=1 正确（JS-20260807-04 · 2026-08-07） |
