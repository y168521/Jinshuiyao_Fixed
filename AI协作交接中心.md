...
| W63补122 | GitHub通道自愈(域名级封堵自动绕行+10分钟计划任务) 2026-09-19 | WorkBuddy | 已完成 | 承 W63补121: 用户 20 分钟后点「New SSH key」又超时 → 复盘发现**不是"某个 IP 挂了", 而是对 `github.com` 的域名级封堵间歇性开合**。铁证: 同一时刻 `github.com:443`(含 140.82.112.3/113.3/116.3/121.3、20.27.177.113、20.233.83.145 全部候选)一律 TCP 超时, 而同段的 `api.github.com:443`(HTTP 200)、`codeload.github.com:443`、`ssh.github.com:443`、`github.com:22` **全部正常** → 换固定 IP 治不好。该结论用 PowerShell 系统网络栈(Invoke-WebRequest + TcpClient)独立复测确认, 排除沙箱自身代理干扰(测本机网络必须 `curl --noproxy '*'`)。对策=**不停测速、谁通钉谁**: 新增 `模型\环境自愈\GitHub通道自愈.ps1`(候选 IP 池, 每个做 TCP 连接+TLS 指定 SNI 握手+HEAD 取状态码的真实可达性测试, 只认 200/301/302/303/307/308/404; 改写 hosts 时只动自己标记包围的一段, 并先备份 hosts; 全部候选失效则沿用上次可用 IP 等下一个窗口; 有变化才 `ipconfig /flushdns`; 日志 `%LOCALAPPDATA%\Jinshuiyao\logs\github_hosts.log` 自动留最近 500 行)+ 同目录 GBK/CRLF 双击入口 `一键修复GitHub通道.bat`。注册计划任务 `GitHub通道自愈` 每 10 分钟静默(第 4 个自定义任务)。**⚠️关键坑三条**(全部本轮踩过): ①新任务必须 `RunLevel=Highest` —— 旧 3 个任务是 Limited 受限令牌, **写不了 hosts**; 且本机 `ConsentPromptBehaviorAdmin=0` 会自动提权不弹 UAC; ②hosts 的标记行**必须是纯 ASCII** —— 用 ANSI 写 hosts 会把中文标记写成 `?`, 导致标记识别失败、管理块**重复嵌套**(本轮已出现并清理), 生成时间行同理; ③写 hosts 要用 `[IO.File]::WriteAllLines(...,[Text.Encoding]::Default)` —— 用 `Set-Content -Encoding ASCII` 会**破坏你 hosts 里原有的中文条目**, 用 `-Encoding UTF8` 会加 BOM 让 hosts 首行失效; ④`github.githubassets.com` 根路径**本来就回 404**, 判定「链路活」必须把 404 计入, 否则会把好的 IP 误判失效而沿用坏的。验证: 第 3 次试跑时 github.com 恰好恢复 → 抓到并钉 `20.27.177.113`; 走正常解析 `https://github.com/` **HTTP 200**、`/settings/ssh/new` 302; 标记行计数 BEGIN/END 各 1(重复嵌套已清); 计划任务 `Hidden=True`/`RunLevel=Highest`/`State=Ready`, `Start-ScheduledTask` 实跑 **LastTaskResult=0**, 且该次运行自动把 avatars/raw 也升级到了可用 IP。⚠️注: 本脚本在 git 仓库**之外**(`模型\环境自愈\`, 随坚果云同步、不进版本库), 与桌面各 bat 同类; 推送本身不受影响 —— `github.com:22` 始终可用, 公钥一旦加入即可推 | JS-20260919-07 |
