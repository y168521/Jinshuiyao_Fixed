...
| W63补69 | 基金API契约测试11项+全项目三层深度盘点+成熟模式调研+诊断与重构规划报告 2026-08-12 | opencode | 已完成 | 用户决策"补契约测试+重新看整个项目+上网调研成熟方案". 交付: ①新增tests/unit/test_fund_api_contract.py 11项契约测试(FakeHandler模式, 覆盖pool三态/nav-series四态含mock_count与period截断/holdings四态含available=false诚实降级与fetch触发), 实测11 passed+相邻fund/server相关106 passed零回归; ②3路并行explore agent深度盘点: server/frontend层(26文件)+domains/engines/core层(59文件)+tools/流程/测试层(40工具60测试), 输出P1/P2/P3分级问题清单; ③6轮网上调研(契约测试Pact/mock-drift、Agent Registry 2026 Field Guide、EDDOps arXiv:2607.00345、工具4阶段废弃、TechDebt.solutions债务治理、Strangler Fig、Boring Architecture); ④产出deliverables/全项目诊断与重构规划_2026-08-12.md. 核心结论: 不推倒重来(Strangler Fig渐进), 5大根因=无单一真源(基金名单3套已合一但fund/backtest仍双份实现+3组双副本页面)/注册表层形同虚设(agent_hub 305行0引用、registry.init_domains从未调用、engines注册表audit错位、kb_engine 769行0引用、lottery analyze状态桩)/防空壳靠事后人肉(PENDING无到期日、AI审查fail-open、code_health_gate永不阻断)/自动化断线10天(同步.ps1无人调度、审计留痕从未落盘、CI纯装饰、gate--e2e引用不存在脚本、mirror_closeout调错路径、hook规范源缺page_api_lint)/无验收标准DoD. 规划3批: P0止血(修断线自动化+审计接线+hook同步+删死路由双副本+建技术债务登记册)→P1单一真源+注册表治理(agent_hub三选一/registry删接/engines修错位/fund-backtest合一/football诚实降级)→P2防空壳闭环(彩票4真引擎+检查器强制mode标记+DoD清单). ⚠️①agent_hub/registry/football兜底等"删vs接线"决策点需用户拍板; ②契约测试为第一批, 其余各批按报告分次执行 | JS-20260812-05 |
