...
本脚本做的事（只补事实，不改口径之外的东西）：
  ...
  4. 输出「旧 hits vs 新 hits」差异报告，并**按方向规律定性**差异成因
     （单向偏移 = 口径差异；随机双向且跨实体 = 串写污染）。
...
def _tally(p, idx, stat, delta_by_lot, by_lot_old, by_lot_new,
           period_lots, samples, limit_samples):
    """处理单条记录并更新统计（从 analyze 抽出，避免主循环函数过长）"""
    from utils.number_utils import count_match

    lot, per = p.get("lot"), p.get("period")
    if p.get("actual"):
    ...
    try:
        match_count, _is_hit = count_match(lot, p.get("nums", ""), actual)
    except Exception as e:
        match_count = None
        samples.append({"kind": "口径计算异常", "lot": lot, "period": per, "err": str(e)})

    old_hits = p.get("hits")
    if match_count is not None and old_hits != match_count:
        stat["hits_changed"] += 1
        delta_by_lot[lot][match_count - (old_hits or 0)] += 1
        if len(samples) < limit_samples:
            samples.append({"kind": "命中数变化", "lot": lot, "period": per,
                            "old": old_hits, "new": match_count,
                            "pred": p.get("nums", ""), "actual": actual})
...
def analyze(limit_samples=15):
    ...
    for p in preds:
        if isinstance(p, dict):
            _tally(p, idx, stat, delta_by_lot, by_lot_old, by_lot_new,
                   period_lots, samples, limit_samples)
...
def _sec_header():
    return [
        "# 历史预测「开奖号回填 + 命中重算」演练报告",
        "",
        "- 生成时间：2026-09-22（JS-20260922-01）",
        "- 脚本：`scripts/backfill_prediction_actual.py`（**默认只读**，`--apply` 才写盘）",
        "- 口径：`utils.number_utils.count_match`（全系统唯一真源，JS-20260917-02）",
        "",
    ]


def _sec_conclusions(stat, applied):
    L = ["## 一、结论先行", ""]
    L.append("1. `predictions.json` 共 **%d 条**，其中 **%d 条（%.0f%%）** 能在 `lot_data` 里反查到开奖号，"
             % (stat["total"], stat["matched"], 100.0 * stat["matched"] / max(1, stat["total"])))
    ...
    if applied:
        L.append("5. **已执行 `--apply`**：补 `actual` %d 条、修正 `hits` %d 条、跳过 %d 条；"
                 "备份在 `predictions.json.bak_backfill`。"
...
def _sec_rate_table(by_lot_old, by_lot_new):
    L = ["## 二、各彩种命中率：旧值 vs 重算后", "",
         "| 彩种 | 已开奖期数 | 旧命中率（hits>0） | 重算后命中率 | 变化 |",
         "|---|---|---|---|---|"]
    for lot in sorted(by_lot_new, key=lambda x: -by_lot_new[x]["n"]):
        ...
        try:
            ...
            d = "—"
        L.append("| %s | %d | %s | %s | %s |"
                 % (lot, n["n"], _fmt_pct(o["hit"], o["n"]), _fmt_pct(n["hit"], n["n"]), d))
    L.append("")
    return L


def _sec_lot_data(per_lot):
    L = ["## 三、lot_data 可索引期数（回填的数据来源）", "",
         "| 彩种 | 可索引期数 |", "|---|---|"]
...
def _judge_delta(d):
    """按差值方向判读成因（单向偏移=口径差异，随机双向=串写）"""
    up = sum(v for k, v in d.items() if k > 0)
    down = sum(v for k, v in d.items() if k < 0)
    if up and not down:
        return "旧值系统性**偏低**（旧口径漏统计）"
    if down and not up:
        return "旧值系统性**偏高**（旧口径多算，疑似把蓝球计入）"
    return "双向都有（旧口径对红/蓝或分区的处理与新口径不同）"


def _sec_qualitative(stat, delta_by_lot):
    L = ["## 四、差异定性：是「串写」还是「口径」？（重要，别想当然）", ""]
    ...
    L.append("- 若是串写，方向应随机且**波及所有彩种**（跨彩种同期号的期号多达 %d 个，串写的前提确实存在）。"
             % stat["cross_period"])
    L.append("")
    ...
    for lot in sorted(delta_by_lot, key=lambda x: -sum(delta_by_lot[x].values())):
        ...
        L.append("| %s | %d | %s | %s |" % (lot, sum(d.values()), d, _judge_delta(d)))
    L.append("")
    L.append("> **诚实的边界**：旧值到底出自哪一套代码（GUI `main_window.py` 多球种口径？"
             "还是 JS-20260917-02 之前的老 `review()`？）")
...
def _sec_samples(samples):
    L = ["## 五、命中数被修正的记录（前 %d 条样例）" % len(samples), ""]
    rows = [s for s in samples if s.get("kind") == "命中数变化"]
    if rows:
...
def _sec_caveats():
    return [
        "## 六、本脚本的保守边界（刻意不做的事）",
        "",
        "- **不改 `reviewed`**：改它会动 GUI 的【待复盘】计数，属行为变更，须人工拍板。",
        "- **不改 `hit_type` / `coverage` / `confidence`**：只补事实（`actual`）与重算命中数（`hits`）。",
        "- **查不到开奖号就跳过**：绝不猜、绝不填 0 充数（诚实口径）。",
        "",
    ]
...
def render_report(stat, samples, by_lot_old, by_lot_new, per_lot, delta_by_lot, applied=None):
    L = []
    L.extend(_sec_header())
    L.extend(_sec_conclusions(stat, applied))
    L.extend(_sec_rate_table(by_lot_old, by_lot_new))
    L.extend(_sec_lot_data(per_lot))
    L.extend(_sec_qualitative(stat, delta_by_lot))
    L.extend(_sec_samples(samples))
    L.extend(_sec_caveats())
    return "\n".join(L)
    ...
    for a in sys.argv:
        if a.startswith("--limit="):
            try:
                ...
                # 原为 `except Exception: pass`（静默吞）；参数写错时用户会莫名只看
                # 到 15 条样例，改为明确提示并收窄异常类型（JS-20260922-01）
                print("[backfill] --limit 不是整数（%s），沿用默认值 %d"
                      % (a.split("=", 1)[1], limit))
